...
{ // добавление сегмента в хвост списка (змейки)
    if (el == NULL)
    {
        return;
    }
...
{ // генерация еды на поле
    ...
    { // проверка что еда не внутри тела змейки
...
bool appleNotInHead(elem *head)
{ // проверка события что змейка съела яблоко и прирост тела
    if (head->x == food_x && head->y == food_y)
    { // если координаты головы и координаты еды совпадают
        if (size == 25)
        { // проверка на победу
            endGameCondition = 0;
            return true;
...
{ // поворот головы змейки
    ...
    { // смена направления движения в зависимости от начального направления и положения соска
...
void moveSnake(uint8_t headDirection)
{ // движение змейки на один тик
...
void lightSnakeAndFood(void)
{
    ...
    for (elem *curr = head; curr != NULL;)
    {
        points[curr->x][curr->y] = 1;
        curr = curr->nextElement;
    }
    points[food_x][food_y] = 1;
    frame();
}
